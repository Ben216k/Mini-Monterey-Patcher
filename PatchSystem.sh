...
if [[ -z "$1" ]] || [[ "$1" == "--detect" ]]; then
    ...
    "$LPATCHES/Scripts/NeededPatches.sh" --rerun "$LPATCHES/PatchSystem.sh" $2
    exit $?
elif echo "$1" | grep '/Volumes/'; then
    echo "Set to detect patches, restarting PatchSystem with NeededPatches..."
    "$LPATCHES/Scripts/NeededPatches.sh" --rerun "$LPATCHES/PatchSystem.sh" $1
    exit $?
fi
...
if [[ ! "$PATCHMODE" == "UNINSTALL" ]]; then
    ...
    if [[ ! -z "$WIFIPATCH" ]]; then
        ...
        /usr/libexec/PlistBuddy -c "Set :IOKitPersonalities:Broadcom\ 802.11\ PCI:IONameMatch:0 $(ioreg -r -n ARPT | grep IOName | cut -c 19- | rev | cut -c 2- | rev)" IO80211FamilyLegacy.kext/Contents/PlugIns/AirPortBrcmNIC.kext/Contents/Info.plist
